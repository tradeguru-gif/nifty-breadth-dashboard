...
EXCHANGE_SEGMENT = "NSE"    # Changed from IDX to NSE (more stable)
...
# ========== ADDED FUNCTIONS (ADX, Time Filter, Volume Spike) ==========
...
def is_market_hours():
    ...
    # Avoid first 15 minutes and last 15 minutes of trading
...
@application.route('/')
def home():
    return jsonify({'message': 'Trade Guru NIFTY Trading API v7.0 (Stable WebSocket + ADX + Time Filter)'})
...
# --------------------------------------------------
# WebSocket connection (synchronous, stable for dhanhq 2.0.2)
# --------------------------------------------------
def start_market_feed():
    ...
    # Initialize the feed - synchronous, will block inside run_forever()
    ...
    feed.run_forever()   # This blocks and keeps the connection alive
...
@application.route('/')
def home():
    return jsonify({'message': 'Trade Guru NIFTY Trading API v7.0 (Stable WebSocket)'})
