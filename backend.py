...
market_signal = {
    ...
    "volume": 0, "volume_avg": 0, "oi_signal": "NEUTRAL",
    "timestamp": ""
}
...
market_state = {
    ...
    "volatility": "NORMAL", "alert": "NONE", "signal_duration_minutes": 0,
    "trend_1min": "SIDEWAYS", "trend_5min": "SIDEWAYS",
    "trend_10min": "SIDEWAYS", "trend_15min": "SIDEWAYS",
    "trend_20min": "SIDEWAYS", "timeframe_agreement": 0
}
...
signal_memory = {
    "current_action": "HOLD", "current_signal_type": "NONE",
    "signal_start_time": None, "last_confirmed_action": "HOLD",
    "confirmation_count": 0, "required_confirmations": 2,
    "min_signal_duration_seconds": 180, "max_sideways_duration": 600
}

timeframe_history = {
    "1min": deque(maxlen=60), "5min": deque(maxlen=20),
    "10min": deque(maxlen=15), "15min": deque(maxlen=10),
...
def watchdog_thread():
    while True:
        ...
        if last_tick_time["value"] > 0 and elapsed > 90:
            logger.warning(f"[WATCHDOG] No tick for {elapsed:.0f}s. Reconnecting...")
            try:
                rest_price_update()
            except:
                pass
...
def rest_price_update():
    ...
    if ce_ltp:
        ...
        logger.info(f"[REST FALLBACK] CE: {ce_ltp}")
    if pe_ltp:
        latest_ticks["pe_price"] = pe_ltp
        latest_ticks["pe_timestamp"] = datetime.now().isoformat()
        logger.info(f"[REST FALLBACK] PE: {pe_ltp}")

# ============================================================
# FEATURE #5: Tick Processing Worker
# ============================================================
def tick_worker():
    while True:
        try:
...
def process_tick_data(data):
    ...
    try:
        ...
        logger.info(f"[WORKER] Processing {len(ticks)} ticks. Queue size: {tick_queue.qsize()}")
        ...
        for tick in ticks:
            ...
            if now - last_minute_snapshot["time"] >= 60:
                ...
                snapshot = {
                    "time": now, "price": avg_price,
                    "ce": latest_ticks["ce_price"], "pe": latest_ticks["pe_price"],
                    "ce_volume": real_volume["ce"], "pe_volume": real_volume["pe"]
                }
...
def get_nifty_pcr_enhanced():
    ...
    try:
        ...
        logger.warning(f"[PCR] NSE failed: {e}")
    if raw_pcr is None:
        try:
...
# ============================================================
# FEATURE #10: Enhanced Anti-Flip - CORRECTED
# ============================================================
def enhanced_anti_flip_logic(raw_action, raw_confidence, signal_type):
    ...
    if in_cooldown and raw_action != current_action and current_action != "HOLD":
        logger.info(f"[ANTI-FLIP] Cooldown ({time_since_last_change:.0f}s). Blocking {raw_action}")
    ...
    if current_action in ["HOLD", "WAITING"]:
        if raw_action != "HOLD" and anti_flip["consecutive_non_hold"] >= signal_memory["required_confirmations"]:
            ...
            anti_flip["consecutive_non_hold"] = 0
            signal_memory["signal_start_time"] = now
...
# ============================================================
# WEBSOCKET CALLBACKS - DEFINED BEFORE start_angel_websocket()
# ============================================================
...
def on_ws_data(wsapp, message, *args):
    try:
        if isinstance(message, str):
            ...
            data = message
        logger.info(f"[WS] Tick received. Queue size before: {tick_queue.qsize()}")
        try:
            tick_queue.put_nowait(data)
            logger.info(f"[WS] Tick enqueued. Queue size after: {tick_queue.qsize()}")
        except queue.Full:
...
def run_signal_engine(ce_price, pe_price, price_list):
    ...
    timeframe_score = 0
    if trend_1min == "BULLISH": timeframe_score += 10
    if trend_5min == "BULLISH": timeframe_score += 10
    if trend_10min == "BULLISH": timeframe_score += 10
    if trend_15min == "BULLISH": timeframe_score += 10
    if trend_20min == "BULLISH": timeframe_score += 10

    bearish_timeframe_score = 0
    if trend_1min == "BEARISH": bearish_timeframe_score += 10
    if trend_5min == "BEARISH": bearish_timeframe_score += 10
    if trend_10min == "BEARISH": bearish_timeframe_score += 10
    if trend_15min == "BEARISH": bearish_timeframe_score += 10
    if trend_20min == "BEARISH": bearish_timeframe_score += 10

    tech_bullish = 0
    tech_bearish = 0

    if 55 < rsi < 75: tech_bullish += 10
    elif rsi > 75: tech_bullish += 3
    elif 40 < rsi < 55: tech_bullish += 5

    if 25 < rsi < 45: tech_bearish += 10
    elif rsi < 25: tech_bearish += 3
    elif 45 < rsi < 60: tech_bearish += 5

    if macd_hist > 0 and macd_line > 0: tech_bullish += 10
    elif macd_hist > 0: tech_bullish += 6
    elif macd_hist < 0 and macd_line < 0: tech_bearish += 10
    elif macd_hist < 0: tech_bearish += 6

    if pcr < 0.9: tech_bullish += 10
    elif pcr < 1.0: tech_bullish += 7
    elif pcr > 1.3: tech_bearish += 10
    elif pcr > 1.2: tech_bearish += 7

    if volume_trend == "INCREASING":
        if bullish_count > bearish_count: tech_bullish += 10
        elif bearish_count > bullish_count: tech_bearish += 10

    avg_price = (ce_price + pe_price) / 2
    if avg_price > vwap and avg_price > ema_slow: tech_bullish += 10
    elif avg_price > vwap or avg_price > ema_slow: tech_bullish += 5
    elif avg_price < vwap and avg_price < ema_slow: tech_bearish += 10
    elif avg_price < vwap or avg_price < ema_slow: tech_bearish += 5
    ...
    if total_bullish >= total_bearish and total_bullish >= effective_consider:
        ...
        if timeframe_agreement >= 4 and raw_confidence >= effective_strong_buy:
            raw_action = "TRENDING: CE"; signal_type = "TRENDING"
        elif raw_confidence >= effective_strong_buy:
            raw_action = "STRONG BUY CE"; signal_type = "MOMENTUM"
        elif raw_confidence >= effective_buy:
            raw_action = "BUY CE"; signal_type = "MOMENTUM"
        elif raw_confidence >= effective_consider:
            raw_action = "CONSIDER CE"; signal_type = "MOMENTUM"
        else:
            raw_action = "HOLD"; signal_type = "NONE"
    elif total_bearish > total_bullish and total_bearish >= effective_consider:
        raw_confidence = total_bearish
        if timeframe_agreement >= 4 and raw_confidence >= effective_strong_buy:
            raw_action = "TRENDING: PE"; signal_type = "TRENDING"
        elif raw_confidence >= effective_strong_buy:
            raw_action = "STRONG BUY PE"; signal_type = "MOMENTUM"
        elif raw_confidence >= effective_buy:
            raw_action = "BUY PE"; signal_type = "MOMENTUM"
        elif raw_confidence >= effective_consider:
            raw_action = "CONSIDER PE"; signal_type = "MOMENTUM"
        else:
            raw_action = "HOLD"; signal_type = "NONE"
    else:
    ...
    market_state.update({
        ...
        "alert": final_action, "signal_duration_minutes": signal_duration,
        "trend_1min": trend_1min, "trend_5min": trend_5min,
        "trend_10min": trend_10min, "trend_15min": trend_15min,
        "trend_20min": trend_20min, "timeframe_agreement": timeframe_agreement
    })
    ...
    institutional_state.update({
        ...
        "volume_trend": volume_trend, "delta": delta, "gamma": gamma, "theta": theta, "vega": vega,
    ...
    market_signal.update({
        ...
        "vwap": round(vwap, 2), "atr": round(atr, 2), "ema_fast": round(ema_fast, 2),
        "ema_slow": round(ema_slow, 2), "delta": delta, "gamma": gamma, "theta": theta, "vega": vega,
        "volume": round(volume_ratio * 100, 0), "volume_avg": 100,
        "oi_signal": institutional_state["oi_buildup"], "timestamp": datetime.now().isoformat()
    })
...
@app.route("/")
def home():
    return jsonify({"status": "online", "message": "Nifty Alpha Engine v2.1 - Production Ready"})
...
def start_background_engine():
    ...
    if not engine_started:
        ...
        logger.info("   - FIXED: WebSocket callbacks defined before use")
