...
def on_ws_data(wsapp, message, *args):
    global tick_counter, last_tick_time, latest_ticks, spot_price_history, ce_price_history, pe_price_history, sws
    last_tick_time = time.time()
    try:
...
def start_angel_websocket():
    global CE_TOKEN, PE_TOKEN, ATM_STRIKE, market_signal, sws, ws_running
    ...
    while True:
        try:
            ...
            if not obj or not feed_token:
                logger.error("Authentication variables missing. Re-attempting connection...")
                time.sleep(10)
                continue
            ...
            if CE_TOKEN and PE_TOKEN and not ws_running:
                logger.info(f"Initializing SmartWebSocketV2 Pipeline for ATM Strike {ATM_STRIKE}")
                
                # Using SmartWebSocketV2 to match your explicit parsing/patching infrastructure
...
# Spawn background execution context safely during loading setup step
