# backend.py - Nifty Options Signal Engine using Angel One SmartAPI WebSocket
...
# --------------------------------------------------
# Global state for ATM option tokens (to be fetched dynamically)
# --------------------------------------------------
CE_TOKEN = None
PE_TOKEN = None
current_strike = None
...
# --------------------------------------------------
# Helper: Nifty spot (cached) – for ATM strike calculation
# --------------------------------------------------
...
# --------------------------------------------------
# Load Angel One instrument master to find ATM option tokens
# --------------------------------------------------
def load_angel_instruments():
    url = "https://margincalculator.angelone.in/OpenAPI_File/files/OpenAPIScripMaster.json"
    try:
        resp = requests.get(url, timeout=30)
        resp.raise_for_status()
        df = pd.DataFrame(resp.json())
        logger.info(f"Loaded {len(df)} instruments from Angel One")
        return df
    except Exception as e:
...
def get_atm_option_tokens():
    """Return CE and PE tokens for current ATM strike (nearest weekly expiry)"""
    ...
    df = load_angel_instruments()
    if df is None:
        return None, None

    # Filter for NIFTY options (exchange segment = NFO)
    ...
    # Find ATM contracts for that expiry
    ...
    if ce_row.empty or pe_row.empty:
        logger.warning(f"No ATM contracts found for strike {atm_strike}")
        return None, None
...
def on_ws_message(wsapp, message):
    ...
    try:
        ...
        # Angel One sends a list of ticks
        for tick in data:
            token = str(tick.get('tk'))
            ltp = tick.get('ltp', 0) / 100  # Angel One sends price in paise
            if token == CE_TOKEN:
            ...
            # When both prices are available, run signal engine periodically
...
def start_angel_websocket():
    global feed, ws_running, CE_TOKEN, PE_TOKEN
    # First, authenticate to get feed token
    try:
        ...
        logger.info("Angel One authenticated, feed token obtained")
    except Exception as e:
        logger.error(f"Authentication error: {e}")
        return

    # Get current ATM option tokens
    CE_TOKEN, PE_TOKEN = get_atm_option_tokens()
    if not CE_TOKEN or not PE_TOKEN:
        logger.error("Could not fetch ATM option tokens. Retrying in 60s...")
        time.sleep(60)
        return
...
# --------------------------------------------------
# Signal engine (identical to your previous working version)
# --------------------------------------------------
...
def start_background_engine():
    ...
    thread.start()
    logger.info("✅ Angel One WebSocket engine started")
...
@app.route("/")
def home():
    return jsonify({"status": "online", "message": "Angel One NIFTY Signal Engine"})
