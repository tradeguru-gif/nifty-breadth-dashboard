...
# --------------------------------------------------
# Get Nifty spot price from NSE (with caching)
# --------------------------------------------------
def get_nifty_spot():
    """Fetch Nifty spot from NSE's official API (raw, no cache)"""
    try:
...
def get_nifty_spot_cached():
    """Fetch Nifty spot with caching to reduce NSE API calls"""
    now = time.time()
    if now - spot_cache["timestamp"] < CACHE_TTL and spot_cache["value"] is not None:
        logger.debug("Returning cached spot price")
...
def home():
    return jsonify({
        "status": "online",
        "message": "NIFTY ATM API Running (cached spot)"
    })
