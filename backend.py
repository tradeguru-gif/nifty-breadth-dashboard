...
# ============================================================
# RISK MANAGEMENT & AUXILIARY MODELS
# ============================================================
class RiskManager:
...
def save_tick(token, price, volume, bid, ask, oi):
    try:
        ...
        logger.error(f"Database write exception on tick storage: {e}")
...
def get_auth_token():
    ...
    try:
        ...
        if not session.get("status"):
            logger.error("Angel One authentication failed. Check your environment parameters.")
            return None, None, None
        auth_token = session["data"]["jwtToken"]
        feed_token = obj.getfeedToken()
        auth_cache.update({"token": auth_token, "feed_token": feed_token, "timestamp": now, "obj": obj})
        logger.info("Angel One API Auth token successfully refreshed")
        return auth_token, feed_token, obj
    except Exception as e:
        logger.error(f"Auth wrapper runtime error: {e}")
        return None, None, None

# ============================================================
# DATA SOURCING PIPELINES (CRITICAL FIX: REMOVED BLOCKED SCRAPERS)
# ============================================================
def get_nifty_spot():
    """Fetches spot index valuation using broker APIs to fully bypass Cloudflare blocks."""
    _, _, obj = get_auth_token()
    if not obj:
        return None
    try:
        # '99926000' represents the institutional static scrip master token ID for Nifty 50 Index
        response = obj.ltpData("NSE", "NIFTY", "99926000")
        if response.get("status") and response.get("data"):
            return float(response["data"].get("ltp", 0))
    except Exception as e:
        logger.error(f"Failed handling institutional index extraction: {e}")
    return None
...
def get_nifty_pcr():
    """Derives a synthetic PCR metric from running ticks history to prevent network deadlock loops."""
    global ce_oi_history, pe_oi_history
    if ce_oi_history and pe_oi_history:
        ce_sum = sum(ce_oi_history)
        pe_sum = sum(pe_oi_history)
        if ce_sum > 0:
            pcr_calc = round(pe_sum / ce_sum, 2)
            pcr_cache["value"] = pcr_calc
            return pcr_calc
...
def get_current_atm_tokens():
    ...
    if not spot:
        logger.warning("Could not pull Spot Index values. Delaying active contract generation...")
        return None, None
    ...
    try:
        ...
        logger.info(f"Tokens resolved: CE={CE_TOKEN}, PE={PE_TOKEN} for strike={ATM_STRIKE}")
        return CE_TOKEN, PE_TOKEN
    except Exception as e:
        logger.error(f"Error compiling scrip master parameters: {e}")
        return None, None

# ============================================================
# TECHNICAL ANALYSIS CORE MATHEMATICS
# ============================================================
...
# ============================================================
# SIGNAL PROCESSING CALCULATIONS ENGINE
# ============================================================
...
# ============================================================
# WEBSOCKET STREAM DATA INTERFACES
# ============================================================
def on_ws_open(wsapp):
    global sws
    logger.info("Market Live Streaming pipeline verified online.")
    if sws and CE_TOKEN and PE_TOKEN:
        try:
            sws.subscribe("tradeguru_001", 1, [{"exchangeType": 2, "tokens": [CE_TOKEN, PE_TOKEN]}])
            logger.info(f"Subscribed targets assigned: CE={CE_TOKEN}, PE={PE_TOKEN}")
        except Exception as e:
            logger.error(f"Subscription stream assignment exception: {e}")
...
def on_ws_close(wsapp, close_status_code, close_msg):
    global ws_running
    ws_running = False
    logger.warning(f"WebSocket Channel disconnected: {close_status_code} | Msg: {close_msg}")
...
def on_ws_data(wsapp, message, *args):
    ...
    try:
        ...
        for tick in ticks:
            ...
            if token == CE_TOKEN:
                latest_ticks.update({"ce_price": ltp, "ce_volume": vol, "ce_bid": bid, "ce_ask": ask, "ce_oi": oi})
                ...
                latest_ticks.update({"pe_price": ltp, "pe_volume": vol, "pe_bid": bid, "pe_ask": ask, "pe_oi": oi})
            ...
            if now - last_minute_snapshot["time"] >= 60:
                ...
                last_minute_snapshot.update({"time": now, "price": avg_price})
        ...
        logger.error(f"Streaming Engine callback parse failure: {e}", exc_info=True)

# ============================================================
# SUPERVISOR BACKGROUND LIFECYCLE WRAPPER
# ============================================================
def start_angel_websocket():
    global ws_running, CE_TOKEN, PE_TOKEN, sws, last_tick_time
    retry_delay = 10
    
    # Let Flask spin up completely before performing heavy initialization networks
    logger.info("Pausing background runner briefly to ensure clean server registration...")
    time.sleep(15)
    
    while engine_active:
        try:
            if not is_market_open():
                logger.info("Market session is inactive. Daemon thread running idle checks...")
                time.sleep(60)
                continue
            ...
            logger.info("Attempting connection to Angel Live Market Streams...")
            sws.connect()
            
            while ws_running and engine_active:
                time.sleep(1)
                if time.time() - last_tick_time > 90:
                    logger.warning("No tracking ticks captured inside 90s interval. Triggering re-connect...")
                    break
            ...
            logger.error(f"Core runtime supervisor tracking error: {e}")
            time.sleep(retry_delay)
            retry_delay = min(retry_delay * 2, 60)

# ============================================================
# FLASK WEB INTERFACES & PLATFORM BINDINGS
# ============================================================
@app.route("/", methods=["GET", "HEAD"])
def home():
    """Explicitly accepts HEAD/GET root checks to pass Render load balancer sweeps."""
    return jsonify({
        "status": "healthy",
        "message": "Ultimate Signal Engine v6.0 Core Operational",
...
def live_signals():
    """Exposes comprehensive state structures to the frontend system layer."""
    return jsonify({
...
def health_check():
    return jsonify({"status": "OK", "uptime_heartbeat": True}), 200

def start_backend_services():
    t = threading.Thread(target=start_angel_websocket, daemon=True)
    t.start()

# Launch worker pipelines cleanly detached from context setup execution frameworks
