# === VERSION 9 - MULTI-INDEX SENTIMENT ENGINE (PRESERVES ORIGINAL SOCKET LOGIC) ===
import sys
import logging
import os
import time
...
INDEX_CONFIG = {
    ...
    "MIDCPNIFTY": {
        ...
        "min_premium": 10, "max_premium": 5000, "atm_strike_multiple": 50,   # changed from 25
        "option_exchange": "NFO"
    },
    "SENSEX": {
...
banknifty_history = deque(maxlen=1000)   # kept for correlation
...
# -------------------------------
#  TECHNICAL INDICATORS (identical to original)
# -------------------------------
...
# -------------------------------
#  AUTH & TOKEN FUNCTIONS (ORIGINAL STYLE)
# -------------------------------
        ...
        if resp.get("status") and resp.get("data"):
            ...
            if index_name == "MIDCPNIFTY" and (ltp < 5000 or ltp > 25000):
                ...
                if search and search.get("status") and search.get("data"):
                    ...
                    ltp2 = obj.ltpData("NSE", "MIDCPNIFTY", token)
                    if ltp2.get("status") and ltp2.get("data"):
                        ltp = float(ltp2["data"]["ltp"])
                        if ltp > 100000: ltp /= 100
            return ltp
    except Exception as e:
...
# -------------------------------
#  POSITION SIZING & RISK (kept simple)
# -------------------------------
...
# -------------------------------
#  ORIGINAL WEBSOCKET HANDLERS (PRESERVED)
# -------------------------------
def on_ws_open(wsapp):
    global sws
    logger.info("WebSocket opened")
    # Build subscription list for all active indices and their option tokens
    tokens = []
...
# -------------------------------
#  ORIGINAL REST API POLLER (PRESERVED & EXTENDED)
# -------------------------------
def is_market_open():
    ...
    while True:
        try:
            ...
            for idx in INDEX_CONFIG:
                ...
                if spot and 15000 < spot < 30000 if idx=="NIFTY" else True:   # loose check
...
# -------------------------------
#  FLASK ROUTES (original plus extra)
# -------------------------------
@app.route("/", methods=["GET"])
def home():
    return jsonify({
        "status": "healthy",
        "engine": "Multi-Index Options Bot v9 (Preserved Socket)",
