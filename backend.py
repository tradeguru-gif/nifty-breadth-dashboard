...
# ------------------------------------------------------------
# Environment variables (Angel One)
# ------------------------------------------------------------
...
# Global state
# ------------------------------------------------------------
CE_TOKEN = os.getenv("CE_TOKEN", "60266")    # override via env if needed
PE_TOKEN = os.getenv("PE_TOKEN", "60267")
CE_SYMBOL = ""
PE_SYMBOL = ""
ATM_STRIKE = 0
EXPIRY_DATE = ""

# Price, volume, OI histories
...
# Multi‑timeframe snapshots (1min,5min,10min,15min,20min)
timeframe_history = {
...
# Market data containers (enriched)
market_signal = {
...
def get_nifty_spot():
    """Fetch spot using Angel One LTP (fallback to NSE API)."""
    try:
        auth_token, feed_token, obj = get_auth_token()
        if auth_token:
            # NIFTY 50 ETF token (common) – adjust if needed
            ltp_data = obj.getLTPData("NIFTY", "99926005")
            if ltp_data and ltp_data.get("data"):
                spot = float(ltp_data["data"]["ltp"])
                logger.info(f"Spot via Angel LTP: {spot}")
                return spot
    except Exception as e:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.info(f"Spot via NSE API: {spot}")
            return spot
    except Exception as e:
...
def get_current_atm_tokens():
    """Auto‑fetch ATM CE/PE tokens (optional – can be disabled if tokens are fixed)."""
    global CE_TOKEN, PE_TOKEN, ATM_STRIKE, EXPIRY_DATE
    spot = get_nifty_spot_cached()
    if not spot:
        return CE_TOKEN, PE_TOKEN   # keep existing
    atm_strike = round(spot / 50) * 50
    # Load instrument master from Angel
    try:
    ...
    if not ce.empty and not pe.empty:
        ...
        logger.info(f"Auto‑updated tokens: CE={CE_TOKEN}, PE={PE_TOKEN}")
    return CE_TOKEN, PE_TOKEN
...
def run_signal_engine(ce_price, pe_price, ce_hist, pe_hist, ce_vol_hist, pe_vol_hist):
    ...
    iv_rank = 50  # simplified; can be enhanced
    ...
    # Signal confirmation (persistent)
    now_ts = time.time()
    ...
    if final_action in ["STRONG BUY CE", "BUY CE", "CONSIDER CE BUY"]:
        ...
        # Paper trade entry / trailing stop
    ...
    market_signal.update({
        ...
        "rsi_divergence": rsi_div, "iv_rank": round(iv_rank,2),
...
# ------------------------------------------------------------
# WebSocket callbacks (enhanced)
# ------------------------------------------------------------
def on_open(wsapp):
    global sws
    logger.info("WebSocket Opened")
    if sws:
        sws.subscribe("tradeguru", 1, [{"exchangeType": 2, "tokens": [CE_TOKEN, PE_TOKEN]}])
...
# ------------------------------------------------------------
# Angel One authentication & WebSocket manager
# ------------------------------------------------------------
...
def start_websocket():
    global ws_running, sws, CE_TOKEN, PE_TOKEN
    retry_delay = 30
    while engine_active:
        try:
            # Auto‑fetch fresh ATM tokens each day (optional)
...
def home():
    return jsonify({
        "status": "online",
        "message": "TradeGuru Professional Signal Engine",
