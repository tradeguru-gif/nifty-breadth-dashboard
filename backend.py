...
# ----------------------------------------------------------------------
# GLOBAL STATE (with all locks properly initialized)
# ----------------------------------------------------------------------
...
def should_enter_trade(index_name, action, spot, ce_prem, pe_prem, confidence, quality, regime, adx, vix):
    ...
    if dd_pct >= max_dd:
        if not daily_drawdown[index_name]["dd_warning_sent"]:
            send_telegram_alert(f"⚠️ {index_name}: Max daily drawdown {dd_pct:.1f}% reached.")
...
def manage_existing_position(index_name, spot, ce_prem, pe_prem, atr, vix, greeks_data, prices):
    ...
    if not exit_reason and highest > entry * 1.2:
        trail_sl = highest * 0.85
        if current <= trail_sl:
            exit_reason = f"Trailing SL @ {trail_sl:.2f}"
    ...
    if not exit_reason:
        max_loss = entry * lots * INDEX_CONFIG[index_name]["lot_size"] * 0.6
        if pnl < -max_loss:
...
def execute_exit(index_name, action, entry, exit_price, pnl, lots, atr, vix, exit_reason):
    ...
    if pnl < 0:
        ...
        if safety_state[index_name]["consecutive_sl"] >= 3:
            ...
            send_telegram_alert(f"🔒 {index_name}: Circuit breaker activated (3 consecutive SLs).")
    else:
        safety_state[index_name]["consecutive_sl"] = 0
    ...
    send_telegram_alert(msg)
    logger.info(f"EXIT: {index_name} {action} @ {exit_price:.2f} P&L={pnl:+.0f}")
...
def on_ws_data(wsapp, message):
    ...
    try:
        ...
        if message is None or message == b'\x00' or message == '\x00' or message == b'ping' or message == 'ping' or message == b'':
            logger.debug("Received ping or empty message")
            return
    ...
    while True:
        ...
        if ws_running:
            if now - last_tick_timestamp > 60:
                logger.warning(f"No ticks for 60s - forcing reconnect")
                with _ws_connect_lock:
                    ws_running = False
                if sws:
                    try:
...
def start_angel_websocket_improved():
    ...
    while True:
        try:
            with _ws_connect_lock:
                if ws_running:
                    logger.info("WS already running, skipping new connection")
                    time.sleep(10)
                    continue
                ws_running = False
...
class ConnectionManager:
    ...
    def start(self):
        if self.use_websocket:
            ...
            logger.info("REST-only mode forced (FORCE_REST_MODE=1).")
...
def _load_candle_histories_from_db():
    ...
    logger.info(f"✅ Loaded {total_loaded} historical candles")
            ...
            for attempt in range(max_retries):
                ...
                for idx in INDEX_NAMES:
                    if INDEX_CONFIG[idx].get("is_commodity"):
                        ...
                        tokens = INDEX_TOKENS.get(idx, {})
                        logger.info(f"Token status {idx}: ce={tokens.get('ce_token')}, pe={tokens.get('pe_token')}")
                logger.info(f"Token prefetch attempt {attempt + 1}: {ready}/{total_active} ready")
                ...
                if market_signal[idx].get("alert_message") == "" and market_signal[idx].get("signal") in ("WAITING", ""):
                    ...
                    if candle_len >= 5:
                        try:
                            ...
                            logger.exception(f"run_signal_engine_for_index({idx}) failed")
