# === HYBRID v15.1 – WS FIRST + ALL CRITICAL FIXES ===
import sys
import logging
import os
import time
...
# ----------------------------------------------------------------------
# INDICATORS (all return numeric, with period guards)
# ----------------------------------------------------------------------
...
class CorrelationFilter:
    def __init__(self):
        self.nifty_prices = deque(maxlen=50)        # store actual prices
...
def get_index_spot(index_name):
    """Fetch spot price with debug logging and token check."""
    config = INDEX_CONFIG.get(index_name)
    if not config:
...
def get_mcx_futures_tokens():
    ...
    for idx, cfg in INDEX_CONFIG.items():
        if not cfg.get("active") or not cfg.get("is_commodity"):
            continue
        symbol = cfg["symbol"]
        # Exact match or starts with symbol (to handle suffixes like GOLD21JUNFUT)
...
# ============================================================
# HELPER FUNCTIONS (moved before signal engine)
# ============================================================
...
# ============================================================
# MAIN SIGNAL ENGINE (CORRECTLY DEFINED)
# ============================================================
def run_signal_engine_for_index(index_name):
    ...
    try:
        ...
        # --- Use bid-ask midpoint (equity only) ---
        if not is_commodity:
                ...
                with _signal_state_lock:
                    ...
                    if stop_loss_val is not None and prem <= stop_loss_val:
                        ...
                        if entry_price > 0:
                            ...
                            logger.error(f"Invalid entry_price for {index_name}: {entry_price}")
...
# ============================================================
# WEBSOCKET + WATCHDOGS (with defensive parsing)
# ============================================================
ws_running = False
sws = None
    ...
    try:
        ...
        if isinstance(message, dict):
            ...
            try:
                ...
                logger.warning(f"Failed to decode binary message: {e}, hex={message[:20].hex()}")
                return
        else:
...
def start_angel_websocket_improved():
    ...
    while True:
        try:
            ...
            # Reset retry delay after a successful connection attempt
            retry_delay = 5
    ...
    while True:
        ...
        try:
            ...
            for idx in assets_to_fetch:
                ...
                # 2. For equity, fetch option quotes (LTP, volume, OI, bid, ask)
                if not INDEX_CONFIG[idx].get("is_commodity"):
                    tokens = INDEX_TOKENS.get(idx, {})
                    if tokens.get("ce_token") and tokens.get("pe_token"):
                        try:
            ...
            # Update last_rest_fetch timestamp
            last_rest_fetch = time.time()

            # Run signal engine after fetching
            run_all_signals()

            # Sleep between cycles, but check ws_running periodically
...
# ----------------------------------------------------------------------
# BACKGROUND THREADS (with corrected token readiness check)
# ----------------------------------------------------------------------
    ...
    with _init_lock:
        if not _init_completed:
            ...
            for attempt in range(max_retries):
                refresh_all_tokens()
                # Check correct tokens: for commodities, spot token; for equity, CE/PE tokens
                ready = 0
                for idx, cfg in INDEX_CONFIG.items():
                    if not cfg.get("active"):
                        continue
...
def home():
    return jsonify({
        "status": "healthy",
        "engine": "Hybrid v15.1 – WS First + All Critical Fixes",
...
def health():
    # If markets are closed, report CLOSED instead of DEGRADED
    if not is_market_open() and not is_mcx_open():
        return jsonify({
            "status": "CLOSED",
            "ws_running": ws_running,
