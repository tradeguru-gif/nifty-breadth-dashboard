...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS daily_performance (date TEXT, equity REAL, daily_pnl REAL, drawdown_pct REAL, sharpe REAL, var REAL)""")
    c.execute("""CREATE TABLE IF NOT EXISTS ml_models (id INTEGER PRIMARY KEY, model BLOB, created_at REAL, features TEXT)""")
    conn.commit()
    conn.close()
...
timeframe_candles = {tf: {"open": 0, "high": 0, "low": float("inf"), "close": 0, "active": False} for tf in TIMEFRAMES}
...
def generate_alert_message(action, strength, spot_price, premium, sl, target, factors, confidence):
    ...
    if action == "BUY_CE":
        ...
        return "⏸️ <b>HOLD</b>\n💰 Spot: " + str(spot_price) + "\n📊 Market: Ranging | Confidence: " + str(round(confidence, 1)) + "%"
    return "📊 <b>WAITING</b>\n💰 Spot: " + str(spot_price)
...
def run_signal_engine():
    ...
    for tf in TIMEFRAMES:
        ...
        if len(tf_data) >= 3:
            ...
            c3 = tf_data[-1]["close"]
            if c3 > c2 > c1:
                market_signal["trend_" + tf] = "BULLISH"
            elif c3 < c2 < c1:
                market_signal["trend_" + tf] = "BEARISH"
            else:
                market_signal["trend_" + tf] = "SIDEWAYS"
        else:
            market_signal["trend_" + tf] = "SIDEWAYS"
